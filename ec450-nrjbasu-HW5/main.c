...
//Neeraj Basu
//EC 450, Professor Giles
//Homework #5- Music Player

// define the P1 port bitmask
//----------------------------------
#define	LED				0x01
#define SPEAKER			0x02    // Output pin for the speaker. Contains the TA0 register
...
// The different notes were created by changing the half periods which subsequently changed the frequencies.
//Two octives are defined below in order to Cover both Joy to the World and The Super Marios theme song

#define SIZE_NOTES_ARRAY	26//Define the size of the array containting the notes

#define	_R		1 // A very small frequency that was used to define pauses between notes of the same frequency
...
const unsigned int notes[SIZE_NOTES_ARRAY] = {
		_R, //
...
#define NOTE_MASK   0x1F    // Used in order to isolate the bits containing the notes from the bits containing the duration

#define NUM_DURATIONS	8 //Allocate array containing the durations

//Notes will be scaled using the values from this array. This is how the durations will be
...
#define JOY_TO_WORLD_LENGTH	72 // Length of "Joy to the World" which is 72 notes

unsigned const char song1[JOY_TO_WORLD_LENGTH] = {
		//Addition of Note duration + Note into 8 bits. First four bits represent the duration second four represent the note
...
#define MARIO_LENGTH	418

unsigned const char song2[MARIO_LENGTH] = {
...
//----------------------------------
#define DEFAULT_TEMPO   2.25 				//this tempo will be the default and will be scaled depending on whether we want to speed up
											//or slow down the song
#define FLASH_INTERVAL  30

unsigned char sys_mod = 2;                  // system state, 0 = song is playing, 1 = song is paused, 2 = start of song, 3 = end of song
											//Song will begin as paused until the play button is pressed
unsigned int curr_song_len = JOY_TO_WORLD_LENGTH;  // current song length, default to JTTW

const unsigned char *curr_song = song2;

unsigned int duration_counter = 0;          // counts the number of cycles for the current duration
unsigned int score_counter = 0;             // indexes into the score for each note
unsigned char flash_counter = 0;            // number of cycles per LED flash
...
void pause_song(void);                    // toggle between paused and not-paused
void restart_song(void);                    // reset song from beginning, resets tempo
void play_song_faster(void);                  // increase the song tempo
void play_song_slower(void);                  // decrease the song tempo
void play_JTTW(void);                    // select song 1, restart song from beginning
void play_Mario(void);                    // select song 2, restart song from beginning
...
void pause_song(void) {
    ...
    if (sys_mod != 3) { // condition that makes sure there is no more toggling at the end of song
...
void play_JTTW(void) {
    sys_mod = 2;
    curr_song = song1;
    curr_song_len = JOY_TO_WORLD_LENGTH;
    restart_song();
}
...
// handles all button interrupts, calls the functions declared above depending on button pressed
interrupt void button_handler(void) {
    ...
    if (P1IFG & RESET_BUTTON) {
        ...
        P1IFG &= ~SONG1_BUTTON;
        //press first button to play JTTW
        play_JTTW();
    }
    else if (P1IFG & SONG2_BUTTON) {
        P1IFG &= ~SONG2_BUTTON;
        //press second button to play Mario theme song
        play_Mario();
    }
}
